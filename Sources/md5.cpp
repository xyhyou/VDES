...
{  
    ...
    {  
        ...
        XLIB_MD5Transform(context->state,context->buffer);
        for (i = partlen; i + 64 <= inputlen; i += 64)
        {
            XLIB_MD5Transform(context->state, &input[i]);
        }
        index = 0;          
    }    
    else  
...
{  
    ...
    XLIB_MD5Encode(bits,context->count,8);
    XLIB_MD5Update(context,PADDING,padlen);
    XLIB_MD5Update(context,bits,8);
    XLIB_MD5Encode(digest,context->state,16);
} 
