...
namespace VDES
{
    namespace SQLite
    {
        ...
        {
            int ret = sqlite3_close(apSQLite); // Calling sqlite3_close() with a nullptr argument is a harmless no-op.

            // Add by xyhyou : try to finalize the statements
            while (ret != SQLITE_OK)
            {
                std::this_thread::sleep_for(std::chrono::milliseconds(200));
                ret = sqlite3_close(apSQLite);
            }
        ...
        bool Database::fieldExists(const std::string &fieldName, const std::string &tableName)
        {
            auto filed = fieldName;
            filed.append("%");
            filed.insert(0, "%");

            Statement query(*this, "SELECT COUNT(*) FROM SQLITE_MASTER WHERE NAME = ? AND SQL LIKE ?");
        ...
        bool Database::indexExists(const std::string &indexName, const std::string &tableName)
        {
            Statement query(*this, "SELECT COUNT(*) FROM SQLITE_MASTER WHERE TYPE = 'index' \
                    AND NAME = ? AND TBL_NAME = ?");
            query.bind(1, indexName);
            query.bind(2, tableName);
            (void)query.executeStep();
            return (1 == query.getColumn(0).getInt());
        }

        Statement Database::buildStatement(const std::string &sql)
        {
            return SQLite::Statement(*this, sql);
        }

        bool Database::isOpened(void) const noexcept
        {
            return mSQLitePtr != nullptr;
        }

        void Database::open(void)
        {
            sqlite3 *handle;
            const int ret = sqlite3_open_v2(mFilename.c_str(), &handle, mSqliteFlags, nullptr);
            mSQLitePtr.reset(handle);
            if (SQLITE_OK != ret)
            {
                throw SQLite::Exception(handle, ret);
        ...
        bool Database::isAutoCommitOn(void) const
        {
            return sqlite3_get_autocommit(getHandle()) ? true : false;
        }
